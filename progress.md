...
## OpenCode lifecycle failure and repair — 2026-07-14

- The first fresh parity run, `run_mrk6n8yk0603d6dd1afc`, failed after the former 120-second builder idle deadline. The initial timeout path released the run without explicitly aborting the upstream OpenCode session.
- Core now aborts the OpenCode session before reporting an idle deadline. The official SDK's generated v2 session surface omits abort, so the adapter uses its official stable `session.abort` endpoint with the same session ID. Live route proof returned `LIVE_ABORT_ROUTE PASS`.
- A second run, `run_mrk6ulpg04c2660ce212`, proved that the new five-minute deadline aborted upstream, but also exposed the underlying defect: the adapter requested `limit=250` from OpenCode 1.17.18 even though the API caps the field at 200. Core swallowed each HTTP 400 polling error and misreported the session as merely busy.
- The adapter now uses `limit=200`. Core resets and counts message and permission polling failures, aborts after three consecutive failures, and reports the exact upstream error instead of waiting out the full deadline. The builder deadline defaults to 300000 ms and accepts only `FLOYD_BUILDER_IDLE_TIMEOUT_MS` integers from 30000 through 900000.
- Fresh acceptance run `run_mrk74hxd0485c4b3d1d0` reached `waiting_review` and passed all six parity checks: CLI start, mid-run cockpit attach, live cockpit tokens, cockpit question answer, same-session continuation without state loss, and exactly one Floyd Core PID (`64892`). The two surfaces received 4,395 and 4,316 frames respectively.

## Multi-provider loopback relay — 2026-07-14

- Added the zero-dependency Core `POST /gateway` relay and `FloydModelClient` driver for OpenCode Zen, OpenCode Go, native OpenAI, native Anthropic, and compatible endpoint overrides.
- Core auth uses `x-floyd-token`; this preserves provider `Authorization`, `x-api-key`, and `anthropic-version` headers. CORS preflight accepts loopback origins only.
- Anthropic system messages are lifted into the top-level `system` string. OpenAI and Anthropic streaming frames normalize to Floyd `delta` and `done` SSE events. Upstream non-2xx status codes and payload bytes pass through unchanged.
- The relay owns the outbound `ClientRequest`; client close/abort destroys the upstream response and socket. Five black-box relay tests prove defaults/path selection, dialect translation, normalized streaming, exact 429 echo-through, and active socket destruction. Two SDK tests prove the split authentication boundary, normalized reader behavior, exact 401 preservation, and reader cancellation.
- Initial live Core probe against PID `79258` proved missing Core token -> 401, authenticated relay -> 200, `/v1/chat/completions`, provider bearer preservation, and normalized `delta` plus `done` frames. Final probe against current PID `95968` additionally proved malformed JSON -> 400 and preservation of an upstream `finish_reason: stop` alongside the final token.
